...
export default function PhotoCarousel({ contributions, isAdmin, onDelete }: PhotoCarouselProps) {
  ...
  const handleLike = async (id: string, e: React.MouseEvent) => {
    ...
    if (isPostLiked(id) || likedIds.includes(id)) {
      return; // Already liked by this user
    }
    try {
      markPostAsLiked(id);
          ...
          <div className="relative w-full max-w-sm flex flex-col items-center">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* Actual image */}
                <div className="aspect-square bg-slate-100/80 rounded-xs overflow-hidden border border-slate-100 flex items-center justify-center p-1">
                  <img
                ...
                <div className="absolute bottom-2 right-2 flex items-center gap-1">
                  {(() => {
                    const isLiked = likedIds.includes(photos[currentIndex].id);
                    return (
                      <button
                        onClick={(e) => handleLike(photos[currentIndex].id, e)}
                        className={`p-1.5 rounded-full transition-colors flex items-center gap-1 border border-rose-100 ${
                          isLiked 
                            ? 'bg-rose-100 text-rose-600 cursor-default opacity-90' 
                            : 'bg-rose-50 hover:bg-rose-100 text-rose-500 cursor-pointer'
                        }`}
                        title={isLiked ? "Você já curtiu esta foto!" : "Dar um coração"}
                        disabled={isLiked}
                      >
                        <Heart className={`w-3 h-3 ${photos[currentIndex].likes > 0 || isLiked ? 'fill-rose-500 text-rose-500' : ''}`} />
